...
'---------- Подготовка хеш-таблиц --------------------------------------
'   1. Таблица SFacc (ключ - SF имена целиком)
...
'   2. Таблица SFacc (по компонентам - словам)
...
' ищем пред SF по словам 1С
    ...
    For i = 2 To EOL_Acc1C
        ...
        With Sheets(Acc1C)
            ...
            If accntAddr <> "" Then ' если поле '1С адрес' не заполнено. пропускаем
            
                accntName = .Cells(i, A1C_NAME_COL)
                If accntName <> accntNamePrev Then      ' дедупликация по 1С имени. считаем, что отсортировано
...
'                                + Sheets(SFacc).Cells(CompSNums(CInt(DlgRes)), SFACC_ACCNAME_COL) + "')" _
                            ...
                            With Sheets(AccntUpd)
                                .Cells(EOL_AccntUpd, ACCUPD_SFID_COL) = Sheets(SFacc).Cells(i, A1C_NAME_COL)
                                .Cells(EOL_AccntUpd, ACCUPD_1CNAME_COL) = Sheets(SFacc).Cells(CompSNums(CInt(DlgRes)), SFACC_ACCNAME_COL)
                            End With
                        ElseIf DlgRes = "create" Then
                            ...
                            NewSFaccForm.SFacc.value = Sheets(Acc1C).Cells(i, A1C_NAME_COL)
...
ExitSub:
    ...
    ResultMsg = "ИТОГ: created " + Str(EOL_AdAcc) + " linked " + Str(EOL_AccntUpd)
    ErrMsg TYPE_ERR, ResultMsg
End Sub
...
Function DlgAccChoice(CompSNums, count, idCol, MSG, MSG2)
    ...
    DlgAccChoice = "X"                      ' если связывать не будем - останется так
 '   If MSG2 <> "" Then
        Do
            SFaccountForm.TextBox2.value = MSG + MSG2       ' основной текст
            SFaccountForm.TextBox1.value = ""               ' исходное значение номера - пусто
