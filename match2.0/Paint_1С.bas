...
' -  IsMatch(Val, Col,[iRow],[SheetN],[DB]) - возвращает True, если Val
'                             есть в колонке Col листа SheetN в файле DB
...
'
' - IsMatch(Val, Col,[iRow],[SheetN],[DB]) - возвращает True, если Val есть в колонке Col листа SheetN в файле DB
'   30.6.12
    ...
    If IsNull(DB) Then
        ...
        Else
            iRow = WorksheetFunction.Match(Val, Sheets(SheetN).Columns(Col), 0)
        End If
    Else
        iRow = WorksheetFunction.Match(Val, DB.Sheets(SheetN).Columns(Col), 0)
    End If
    On Error GoTo 0
    If iRow <> 0 Then IsMatch = True
End Function
Sub testIsMatch()
    Dim A As Boolean
    Dim iRow As Integer
    ...
    Set DB_SFDC = Workbooks.Open(F_SFDC, UpdateLinks:=True, ReadOnly:=True)
    A = IsMatch("ООО ""А-Софт""", 2, iRow, SFacc, DB_SFDC)
    DB_SFDC.Close SaveChanges:=False
    A = IsMatch("ООО ""А-Софт""", 9, , 2)
    A = IsMatch("ООО ""А-Софт""", 9)
    A = IsMatch("Рога и Копыта", 9)
End Sub
